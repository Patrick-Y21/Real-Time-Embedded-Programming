...
int main()
{
 ...
 std::cout << "[callback]:state update " << (state ? "Open" : "Close") <<
std::endl;
 });
 callbackManager.registerSensorCallback([](int temp, int hum) {
 std::cout << "[callback] update: " << temp << " C, " << hum << " %" <<
std::endl;
 if(temp > 27) {
 ...
 //<< local_time->tm_sec << std::endl;
		...
		{
			clock_hour = (local_time->tm_hour + clock_hour + (local_time->tm_min +
clock_hour_half)/60)%24;
		...
		if(((clock_hour+clock_hour_half/60)%24 == local_time->tm_hour) && (clock_hour_half ==
local_time->tm_min) && flag_clock)
		{
			beep.set_value(1);
			clock_hour = 0;
			clock_hour_half = 0;
		...
		//std::cout << "clock" << (clock_hour+clock_hour_half/60)%24 << ":" <<
clock_hour_half%60 << std::endl;
 	}
